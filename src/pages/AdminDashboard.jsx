...
function AgentApprovals() {
  ...
  const [form, setForm] = useState({ password: '', pin: '', modules: { realEstate: true, developments: false, mortgage: false, lifestyle: false, luxuryGoods: false } });
  ...
  const doApprove = () => {
    ...
    if (auth.approveAgent) { auth.approveAgent(modal.id, { password: form.password, pin: form.pin, modules: form.modules }); }
    else {
      const p = JSON.parse(localStorage.getItem('pending_agents') || '[]');
      const a = JSON.parse(localStorage.getItem('approved_agents') || '[]');
      const idx = p.findIndex(x => x.id === modal.id);
      if (idx !== -1) { a.push({ ...p[idx], status: 'approved', password: form.password, pin: form.pin, modules: form.modules, approvedAt: new Date().toISOString() }); p.splice(idx, 1); localStorage.setItem('pending_agents', JSON.stringify(p)); localStorage.setItem('approved_agents', JSON.stringify(a)); }
    }
    setModal(null); setForm({ password: '', pin: '', modules: { realEstate: true, developments: false, mortgage: false, lifestyle: false, luxuryGoods: false } }); refresh();
  };
    ...
    <div>
      ...
      {modal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'rgba(15,23,42,0.98)', border: '1px solid rgba(203,166,88,0.4)', padding: '32px', maxWidth: '480px', width: '90%', backdropFilter: 'blur(20px)' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              {[{k:'realEstate',l:'Real Estate'},{k:'developments',l:'Developments'},{k:'mortgage',l:'USA Mortgage'},{k:'lifestyle',l:'Lifestyle'},{k:'luxuryGoods',l:'Luxury Goods'}].map(m => (
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={doApprove} style={{ ...styles.button, ...styles.buttonSuccess, flex: 1, padding: '12px' }}>APPROVE & SAVE</button>
